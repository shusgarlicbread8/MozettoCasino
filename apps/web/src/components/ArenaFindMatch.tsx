...
/**
 * Shared Find Match UI for Texas Hold'em (HU) and Poker (Classic) 6-max.
 * League buy-in is fixed. The platform assigns the table.
 */
...
const PRODUCT = {
  texas: {
    title: "Texas Hold'em",
    subtitle:
      "Heads-up ranked matches. Pick a league and AI profile — the platform finds your opponent. Exactly two seats.",
    lobbyPath: "/v1/arena",
    findPath: "/v1/arena/find-match",
    gameLine: "Texas Hold'em · Heads-Up · Rated",
    rules:
      "Ranked matches are heads-up Texas Hold'em with equal stacks at a fixed buy-in — blinds are engraved as 10% / 5% of buy-in. Opponents are assigned randomly. Same pair is capped at 5 matches per day. Empty tables close after 10 minutes.",
  ...
  classic: {
    ...
    rules:
      "Classic tables seat up to six players with equal buy-ins. Find Match opens a table or fills an open seat. Blinds are 10% / 5% of buy-in. Empty tables close after 10 minutes.",
    altHref: "/poker",
...
export function ArenaFindMatch({ product }: { product: ArenaProduct }) {
  ...
  async function findMatch() {
    ...
    setStatus(
      isOnchain
        ? `Joining ${formatMoney(league.buyIn)} from your Arena Account…`
        : cfg.searching,
    );
    try {
      ...
      if (result.status === "waiting" || result.status === "matching") {
        ...
        // Poll until paired + seated — do not require a second Find Match click.
        for (let i = 0; i < 45; i++) {
          ...
          try {
            ...
            if (again.status === "matching" || (again.tableId && again.waitingForChain && !again.joined)) {
              setStatus(again.message ?? "Match found — opening on-chain…");
              continue;
            }
            ...
            // Transient ticket/exposure races while the pairer opens the session — keep polling.
  ...
  return (
    <main style={{ flex: 1, width: "100%", minWidth: 0, padding: "24px 28px 56px", boxSizing: "border-box" }}>
      ...
      >
        <div>
          <h1 style={{ margin: 0, fontSize: 29, fontWeight: 600, letterSpacing: "-.035em" }}>{cfg.title}</h1>
          <p style={{ margin: "8px 0 0", fontSize: 13.5, color: "#7A7A7A", maxWidth: 540 }}>
            {cfg.subtitle} Buy-in locks from your wallet when matched and returns on settle.
          </p>
          <div style={{ marginTop: 10 }}>
            <Link
              href={cfg.altHref}
              style={{ font: `500 12px ${MONO}`, color: "#00E676", textDecoration: "none", letterSpacing: ".04em" }}
            >
      ...
      >
        ...
        >
          ...
          >
            ...
            {isOnchain && (
              <div style={{ marginTop: 8, font: `400 10px ${MONO}`, color: seamlessEnabled ? "#6DFFB0" : "#FFB86C" }}>
