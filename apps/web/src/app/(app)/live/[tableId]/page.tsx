...
export default function LiveSpectatorPage() {
  ...
  const session = [
    ...
    { k: "CLOCK", v: remaining != null ? `${remaining}s` : "—", color: remaining != null && remaining <= 5 ? color.danger : color.textMuted },
  ];
  ...
  const verifyPath = verifyHref(meta?.onchain_session_id) ?? "/verify";
